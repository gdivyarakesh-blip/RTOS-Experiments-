...
Model a **capacity-limited shared resource** using a FreeRTOS Counting Semaphore shared across three competing tasks, and examine resource contention, concurrent access control, and task interleaving through the SWV ITM Data Console.
...
| Counting Sema.   | N (settable)   | Resource pools   | No (N at a time)  |
+------------------+----------------+------------------+--------------------+
```
...
With semaphore count = **2**, tasks A and B hold it simultaneously, both write to ITM at the same time, producing **mixed output**:
...
This is **intentional, not a defect** — it accurately demonstrates that counting semaphores govern *how many* tasks may concurrently access a resource, but not *the order or atomicity* of those accesses.
