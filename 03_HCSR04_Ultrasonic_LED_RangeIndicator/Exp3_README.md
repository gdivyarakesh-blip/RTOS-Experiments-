...
**Objective:** Interface an HC-SR04 ultrasonic sensor with the STM32F446RE, compute distance using time-of-flight measurements, and indicate range thresholds through the onboard LED and UART output.
...
while (HAL_GPIO_ReadPin(ECHO_GPIO_Port, ECHO_Pin) == GPIO_PIN_SET) {
    numTicks++;
    usDelay(2);   // each iteration ~= 2.8 us
}

// 5. Compute distance
distance = (numTicks + 0.0f) * 2.8 * speedOfSound;

// 6. Send result via UART
...
void usDelay(uint32_t uSec) {
    if (uSec < 2) uSec = 2;
    usTIM->ARR = uSec - 1;   // Load the target count
...
> **Why use direct register access?** `HAL_Delay()` works only at 1 ms resolution — far too coarse for the microsecond-level timing this sensor requires.
...
2. How do registers `ARR`, `EGR`, `SR`, and `CR1` interact inside `usDelay()`?
3. What is the purpose of calling `usDelay(2)` inside the echo pulse measurement loop?
...
The HC-SR04 ultrasonic sensor was successfully interfaced with the STM32F446RE. Distance values were derived using `d = (v * dt) / 2` and transmitted to a terminal via UART. The onboard LED activated whenever the measured distance fell below the 20 cm threshold, confirming correct sensor integration and proximity detection behavior.
