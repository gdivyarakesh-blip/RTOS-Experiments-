...
> No RTOS. No waiting. Just intelligent use of a millisecond timestamp.

---

## Objective

Implement a **non-blocking super loop** that concurrently handles three independent periodic tasks — LED toggling, button polling, and IR sensor sampling — on a single CPU core without any `HAL_Delay()` blocking calls.
...
5. Under what circumstances would a super loop be inadequate, necessitating a full RTOS instead?

---

## Result

The non-blocking super loop successfully executed three independent periodic tasks in parallel without any stalling, demonstrating that `HAL_GetTick()`-based software counters provide a lightweight and effective substitute for RTOS task scheduling in resource-constrained embedded applications.
